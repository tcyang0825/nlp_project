...
if __name__ == "__main__":
    ...
    while True:
        sentence = input("请输入句子（输入quit或exit退出）：")
        if sentence.lower() == "quit" or sentence.lower() == "exit":
            break
        else:
            print('输入文本：{}'.format(sentence))
            # tokenizer = ppnlp.transformers.ErnieTokenizer.from_pretrained("/content/drive/MyDrive/supervised_learning/checkpoints/model_9/")
            ...
            cls_embedding = inner_model.get_pooled_embedding(input_ids=input_ids, token_type_ids=token_type_ids)
            # print('提取特征:{}'.format(cls_embedding))
            recalled_idx, cosine_sims = final_index.knn_query(
                cls_embedding.numpy(), 10)
            print('检索召回')
            for doc_idx, cosine_sim in zip(recalled_idx[0], 1.0 - cosine_sims[0]):
